...
public class TextFileLogServiceImpl implements SystemLogService {
    ...
    public void save(SystemLogSaveRequestDto systemLogSaveRequestDto) {
        Path resourceDirectory = Paths.get("advertisement-service", "src", "main", "resources", "logs", STR."\{systemLogSaveRequestDto.getRecordDateTime().format(CUSTOM_FORMATTER)}.txt");
